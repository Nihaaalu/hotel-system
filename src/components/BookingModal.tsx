...
export default function BookingModal({
  ...
  // Food / Activity Charges State
  const [isFoodActivityCollapsibleOpen, setIsFoodActivityCollapsibleOpen] = useState(true);
  const [isFoodActivityModalOpen, setIsFoodActivityModalOpen] = useState(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-3 bg-gray-950/60 backdrop-blur-xs transition duration-150">
        ...
        <div className="p-3 sm:p-4 overflow-y-auto flex-1 space-y-3 text-xs text-gray-800">
          ...
          {isEditing && loadedBooking ? (
            ...
            <div className="space-y-2.5">
              ...
              {/* 9. FOOD / ACTIVITY CHARGES SECTION */}
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <button
                    type="button"
                    onClick={() => setIsFoodActivityCollapsibleOpen(!isFoodActivityCollapsibleOpen)}
                    className="flex items-center gap-1.5 text-[13px] font-bold text-slate-700 uppercase tracking-wider cursor-pointer hover:text-indigo-600 transition"
                  >
                    <Utensils className="w-3.5 h-3.5 text-indigo-600" />
                    <span>Food / Activity Charges</span>
                    <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isFoodActivityCollapsibleOpen ? 'rotate-180' : ''}`} />
                  </button>
                  <button
                    type="button"
                    onClick={() => setIsFoodActivityModalOpen(true)}
                    className="text-indigo-600 hover:text-indigo-800 font-bold text-[11px] flex items-center gap-1 cursor-pointer"
                  >
                    <Plus className="w-3 h-3" />
                ...
                {isFoodActivityCollapsibleOpen && (
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-2 text-center">
                    <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 pb-1 border-b border-slate-200/60">
                      <span>Linked Charges</span>
                      <span className="px-2 py-0.5 bg-slate-200/70 text-slate-700 text-[10px] font-extrabold rounded-full">0 Charges</span>
                    </div>

                    <p className="text-xs font-semibold text-slate-700 mt-1">
                      No charges linked to this booking.
                    </p>
                    <p className="text-[11px] text-slate-500 leading-relaxed max-w-xs mx-auto">
                      Restaurant bills, Pool charges, Laundry, Room service and other guest services will appear here.
                    </p>

                    <button
                      type="button"
                      onClick={() => setIsFoodActivityModalOpen(true)}
                      className="w-full mt-2 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-extrabold text-xs rounded-xl border border-indigo-200 transition flex items-center justify-center gap-1.5 cursor-pointer shadow-2xs"
                    >
      ...
      {/* FOOD / ACTIVITY CHARGES MODAL */}
      {isFoodActivityModalOpen && (
        <FoodActivityBillModal
          bookingId={loadedBooking?.id || bookingId || null}
          bookings={contextBookings}
          onClose={() => setIsFoodActivityModalOpen(false)}
        />
      )}
    </div>
  );
